...
%Institut National des Sciences Appliques - Rouen
...
\begin{frame}{Architecture classique}
 ...
 \note{
 \begin{itemize}
  \item L'architecture que nous avons mis en oeuvre (PHP $\Leftrightarrow$ C, SQLite $\Leftrightarrow$ ecpg, PostgreSQL) déporté ici sur le serveur
...
\note{
\begin{itemize}
 \item Peut se faire sans ORM (e.g. JDBC) mais réptétion de code + liaison entre objets et tables travail bas niveau
 \item L'idée est d'ajouter une couche supplémentaire qui fait la correspondance (à la main ou via un ORM) entre le modèle objet et le modèle relationnel. Le client ne manipule que des objets. 
\end{itemize}
...
\begin{itemize}
 ...
 \item BookApp.java : l'appli qui réalise les fonctionalités demandées (ajout/suppression/mise à jour Livres)
\end{itemize}
\end{bclogo}
