...
\subsubsection{Exemples de requêtes}
  ...
  MongoDB met à disposition des opérateurs spéciaux permettant d'effectuer des conditions plus complexes : ou (\verb|$or|), supérieur à (\verb|$gt| pour \textit{greater than}), inférieur à (\verb|$lt| pour \textit{lower than}), présent dans le tableau (\verb|$in|)\dots
