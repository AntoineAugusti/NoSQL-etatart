...
\begin{questionmult}{q0}
  ...
  \begin{reponses}
    ...
    \mauvaise{Elle est difficile à mettre en place}
...
\begin{questionmult}{q5}
  ...
  \begin{reponses}
    ...
    \bonne{On doit choisir son type de BDD NoSQL en fonction du besoin}
...
\begin{question}{q7}
  ...
  \begin{reponseshoriz}
     ...
     \mauvaise{Coller les serveurs les uns aux autres}
...
\begin{questionmult}{q13}
   ...
   \begin{reponses}
     ...
     \mauvaise{Stocker durablement des données}
   \end{reponses}
 \end{questionmult}
 }
 \element{idg}{
...
\element{idg}{
  \begin{question}{q17}
    ...
    \begin{reponses}
      \bonne{Un vecteur de nombres qui représente le sens d'un contenu}
      \mauvaise{Un document JSON imbriqué dans un autre document}
      \mauvaise{Une clé de hachage permettant de retrouver une valeur}
      \mauvaise{Un lit superposé pour serveurs}
    \end{reponses}
  \end{question}
}
\element{idg}{
  \begin{question}{q18}
    Quelle est l'opération principale d'une base de données vectorielle ?
    \begin{reponses}
      \bonne{La recherche des plus proches voisins d'un vecteur}
      \mauvaise{La jointure entre plusieurs tables}
      \mauvaise{Le parcours des relations entre des nœuds}
      \mauvaise{L'incrémentation d'un compteur}
    \end{reponses}
  \end{question}
}
\element{idg}{
  \begin{questionmult}{q19}
    Dans une application de RAG (Retrieval-Augmented Generation)\dots
    \begin{reponses}
      \bonne{On recherche des documents pertinents avant d'interroger le LLM}
      \bonne{On calcule l'embedding de la question de l'utilisateur}
      \mauvaise{On réentraîne le LLM à chaque nouvelle question}
      \mauvaise{On envoie toute la base de données au LLM à chaque question}
    \end{reponses}
  \end{questionmult}
}
\element{idg}{
  \begin{question}{q20}
    Pourquoi utilise-t-on des index approximatifs (HNSW, IVF\dots) dans une base vectorielle ?
    \begin{reponses}
      \bonne{Pour gagner beaucoup en vitesse en perdant un peu de précision}
      \mauvaise{Pour garantir de toujours trouver le résultat exact}
      \mauvaise{Pour chiffrer les vecteurs}
      \mauvaise{Parce que les vecteurs ne tiennent pas dans un fichier JSON}
    \end{reponses}
  \end{question}
}
\element{idg}{
  \begin{question}{q21}
    Il faut obligatoirement une base de données vectorielle dédiée pour faire de la recherche sémantique.
    \begin{reponseshoriz}
      \bonne{Faux}
      \mauvaise{Vrai}
    \end{reponseshoriz}
